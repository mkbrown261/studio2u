...
### Remote Recording (free, all engineers)
- **Engineer opt-in** (`/dashboard/profile`): a toggle ("I offer remote recording / mixing") plus a free-text details field ("How does it work?") shown when enabled — free text because remote workflows vary a lot per engineer (live Zoom session while recording vs. file exchange with a turnaround time, etc.).
- **Directory filter** (`/engineers?remote=1`): a "Remote available" checkbox filters the directory to only engineers who've opted in.
- **Visual treatment**: a new custom **teal** accent color (`#3ED8CC`/`#6EE5DB`/`#1E9E93`, added to the Tailwind config in `src/renderer.tsx`) is used exclusively for Remote Recording UI — the only cool color in an otherwise all-warm (gold/wine) palette, so it stands out deliberately. A matching flat-style icon (`icon-remote-128.png`/`icon-remote-64.png`) appears on the directory filter, directory card badges, and the profile page.
- **Public profile** (`/engineers/:id`): a prominent "Remote Available" badge near the engineer's name, plus a details card in the About section (matching the visual pattern of the existing Equipment cards) showing the engineer's own description of how their remote process works.
- Ships **fully free and ungated for every engineer** — not tied to any subscription tier. (`migrations/0010_remote_recording.sql` — `offers_remote`, `remote_details` columns on `engineer_profiles`.)

### Customer Retention (early pass)
- **Account-linked bookings**: `POST /api/bookings` now stamps `customer_user_id` on the booking when the customer is logged in at checkout (via `getSessionUser`) — guest checkout is unaffected and still works with just an email. This is the foundation for real account-based booking history/rebooking (rather than an email-lookup-only model) going forward.
- **"Book This Engineer Again"**: on `/status`, any `completed` booking now shows a button linking straight to `/book/:engineerProfileId` for a fast rebook.
- **Trust/protection copy**: both `/status` and `/book/:engineerId` now show a short note linking to the Refund & Cancellation Policy, reinforcing that sessions booked through Studio2U are protected — part of the broader anti-disintermediation strategy (giving clients and engineers a reason to keep transacting on-platform).
- **Not yet built**: referral/repeat-client credits (leaning toward a Stripe promo-code/coupon model rather than a cash-equivalent wallet, to avoid refund-liability and commission-split edge cases) — deliberately deferred pending more usage data.
...
| `/status?email=` | GET | Customer's booking history + live status + review prompts + "Book Again" on completed sessions |
| `/engineers?remote=1` | GET | Directory filtered to engineers offering remote recording/mixing |
...
- **Engineer Pro subscription** (in active discussion, not yet built): a paid tier for engineers (proposed $15/mo → reduced platform fee, $30/mo → near-zero fee) that would also bundle non-fee perks (featured placement, more portfolio slots, analytics, priority ranking, faster payouts). Deliberately **not started** — open questions still being worked through: the concrete non-fee value proposition, anti-disintermediation mechanics (preventing engineers from using the platform for discovery then transacting off-platform), and whether the top tier should floor at a small fee (e.g. 2%) rather than 0% to avoid fully decoupling engineers from the platform's revenue share.
